...
type ParticleKey = keyof typeof PARTICLES;

const BEAM_PARTICLES: ParticleKey[] = ['proton', 'electron', 'positron', 'phosphorus'];

// Decay channels by collision energy threshold (GeV)
function getDecayProducts(energy: number): ParticleKey[] {
  const products: ParticleKey[] = [];
...
export default function ParticleCollider() {
  ...
  interface BeamParticle {
    ...
    material: { color: { setHex: (hex: number) => void }; opacity: number };
  ...
  const stateRef = useRef({
    phase: 'idle' as 'idle' | 'charging' | 'firing' | 'collision' | 'cascade' | 'cooldown',
    t: 0,
    chargeT: 0,
    beamParticles: [] as BeamParticle[],
    cascadeParticles: [] as ParticleState[],
    trails: [] as Trail[],
    detectorFlash: 0,
  ...
  const [decayLog, setDecayLog] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    // ── Detector rings ──
    const detectorRings: THREE.Mesh[] = [];
    ...
    const trailPool: { line: THREE.Line; positions: Float32Array; head: number }[] = [];
    ...
    const animate = () => {
      ...
      const pA = (PARTICLES as any)[cfg.beamA];
      const pB = (PARTICLES as any)[cfg.beamB];
      ...
      if (s.phase === 'charging') {
        ...
        for (let i = 0; i < 20; i++) {
          ...
          (bp.material as THREE.MeshBasicMaterial).color.setHex(side < 0 ? pA.color : pB.color);
          (bp.material as THREE.MeshBasicMaterial).opacity = 0.3 + 0.7 * progress * (1 - phase);
          bp.scale.setScalar(0.5 + progress * 0.5);
        }

        // Detector rings glow
        for (const ring of detectorRings) {
          (ring.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.5 + progress * 2;
        }
      ...
      if (s.phase === 'firing') {
        ...
        (mainA.material as THREE.MeshStandardMaterial).color.setHex(pA.color);
        (mainA.material as THREE.MeshStandardMaterial).emissive.setHex(pA.color);
        mainA.scale.setScalar(pA.size * 2);
        (mainB.material as THREE.MeshStandardMaterial).color.setHex(pB.color);
        (mainB.material as THREE.MeshStandardMaterial).emissive.setHex(pB.color);
        mainB.scale.setScalar(pB.size * 2);
        (mainA.material as THREE.MeshStandardMaterial).opacity = 1;
        (mainB.material as THREE.MeshStandardMaterial).opacity = 1;
        ...
        (tA.line.material as THREE.LineBasicMaterial).color.setHex(pA.color);
        (tB.line.material as THREE.LineBasicMaterial).color.setHex(pB.color);

        // Update trail positions
        for (const [t, p] of [[tA, mainA], [tB, mainB]] as const) {
      ...
      if (s.phase === 'collision') {
        ...
        (flashMesh.material as THREE.MeshBasicMaterial).opacity = (1 - flashProgress) * 0.9;
        flashMesh.scale.setScalar(0.5 + flashProgress * 4);
        (flashMesh.material as THREE.MeshBasicMaterial).color.setHex(0xffffff);

        // Detector rings flash
        for (const ring of detectorRings) {
          (ring.material as THREE.MeshStandardMaterial).emissiveIntensity = (1 - flashProgress) * 8;
          (ring.material as THREE.MeshStandardMaterial).emissive.setHex(0x88ccff);
        }
      ...
      if (s.phase === 'cascade') {
        ...
        for (const cp of s.cascadeParticles) {
          ...
          const pInfo = (PARTICLES as any)[cp.key];
          const mesh = particlePool[cp.idx];
          if (mesh && cp.idx < MAX_PARTICLES) {
            mesh.visible = cp.life > 0;
            mesh.position.set(cp.x, cp.y, cp.z);
            (mesh.material as THREE.MeshStandardMaterial).color.setHex(pInfo.color);
            (mesh.material as THREE.MeshStandardMaterial).emissive.setHex(pInfo.color);
            (mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.6 * cp.life;
            (mesh.material as THREE.MeshStandardMaterial).opacity = Math.max(0, cp.life);
            ...
            if (trail) {
              ...
              (trail.line.material as THREE.LineBasicMaterial).color.setHex(pInfo.color);
              (trail.line.material as THREE.LineBasicMaterial).opacity = cp.life * 0.4;
        ...
        // Detector rings fade
        for (const ring of detectorRings) {
          (ring.material as THREE.MeshStandardMaterial).emissiveIntensity *= 0.97;
        }
        ...
        if (allDead || s.chargeT > 6) {
          ...
          for (const ring of detectorRings) {
            (ring.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.5;
            (ring.material as THREE.MeshStandardMaterial).emissive.setHex(0x112233);
          }
    ...
    const getXY = (e: MouseEvent | TouchEvent) => {
      const ev = e as any;
      if (ev.touches?.length > 0) return { x: ev.touches[0].clientX, y: ev.touches[0].clientY };
      if (ev.changedTouches?.length > 0) return { x: ev.changedTouches[0].clientX, y: ev.changedTouches[0].clientY };
  ...
  const decayCounts = useMemo(() => {
    ...
    return Object.entries(map)
      .sort((a, b) => b[1] - a[1])
      .map(([key, count]) => ({ key, count, info: (PARTICLES as any)[key] }));
  }, [decayLog]);

  const canFire = phase === 'idle' || phase === 'cooldown';

  const cssHex = (threeHex: number) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100vh', background: '#030608', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <input 
            ...
            aria-label="Collision energy in GeV"
      ...
      <div style={{
        ...
        {(PARTICLES as any)[beamA].symbol} + {(PARTICLES as any)[beamB].symbol} → ?
      </div>
