...
import { initDB } from './utils/pglite-cheo.ts'
...
function CheomaticaApp(): JSX.Element {
  // ────────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    initDB().then(async (db) => {
      console.log('[Cheomatica] PGLite ready, initializing SBT sync...')
      ...
      bridge.start()
      console.log(`[Cheomatica] Sync bridge active — ${sovereignIdentity.alias} is connected to the mesh`)
      setDbReady(true)

      // Check if user has seen welcome before
      const hasSeenWelcome = localStorage.getItem('p31-cheomatica-welcome-shown')
      if (!hasSeenWelcome) {
        setTimeout(() => setShowWelcome(true), 500)
      }
    }).catch(err => {
      console.error('[Cheomatica] Failed to initialize sync:', err)
    })
  }, [sovereignIdentity])

  const handleCloseWelcome = () => {
    localStorage.setItem('p31-cheomatica-welcome-shown', 'true')
  ...
  if (!dbReady) {
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: '48px', marginBottom: '16px' }}>💇</div>
          <div>Initializing Cheomatica...</div>
    ...
    <>
      ...
      {showWelcome && (
        <P31Welcome
          ...
          appName="Cheomatica"
          appDescription="Professional color formulation system for salon operators. Track client formulas, manage developer ratios, and maintain your color history with voice-enabled notes."
          moneyTips={[
            "Track formula costs to optimize pricing",
            "Build a referral network through documented results",
            "Create educational content from your formula database",
            "Offer color consulting services using your tracked data",
            "Sell specialized formulation guides to other stylists"
          ]}
          nextSteps={[
            { energy: 'spoon-deficit', action: 'Add just one client profile' },
            { energy: 'spoon-deficit', action: 'Try the voice dictation feature' },
            { energy: 'moderate', action: 'Import your existing client list' },
            { energy: 'moderate', action: 'Document your first formula' },
            { energy: 'full', action: 'Transfer all your color history' },
            { energy: 'full', action: 'Set up formula templates' },
            { energy: 'any', action: 'Share referral codes with clients' }
          ]}
      ...
      <ReturnRibbon currentApp="cheomatica" />
