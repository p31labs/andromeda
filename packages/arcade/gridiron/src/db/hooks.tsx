...
export function DatabaseProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Add overall timeout for the entire init process
        ...
        setError(err instanceof Error ? err : new Error(String(err)));
        setInitialized(true); // Continue anyway with no DB
      }
    };
    
    // Start init after a small delay to let UI render first
    const timer = setTimeout(init, 100);
    return () => clearTimeout(timer);
  }, []);

  // If init is taking too long, force continue after 8 seconds
  useEffect(() => {
    if (!initialized) {
      const forceTimer = setTimeout(() => {
        console.log('[DatabaseProvider] Force continuing after timeout');
        setInitialized(true);
      }, 8000);
  ...
  return (
    <DatabaseContext.Provider value={{ db, initialized, error } as any}>
      {children}
