// @ts-nocheck — Legacy dashboard, excluded from strict type sweep. Tracked in WCD-L02 parking lot.
/**
...
// @ts-ignore
import SpoonTracker from './ui/SpoonTracker';
// @ts-ignore
...
function generateIcosahedronVertices(radius: number, subdivisions: number) {
...
export default function SpaceshipEarth() {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const sceneRef = useRef<THREE.Scene | null>(null);
  const meshRef = useRef<THREE.InstancedMesh | null>(null);
  const composerRef = useRef<EffectComposer | null>(null);
  const wireMatRef = useRef<THREE.ShaderMaterial | null>(null);
  ...
  const observationStateRef = useRef<Record<string, { collapsed: number; lastObserved: number }>>({}); // nodeId -> { collapsed: 0-1, lastObserved: timestamp }
  const entanglementStateRef = useRef<Record<string, { triggered: number; delay: number }>>({}); // nodeId -> { triggered: timestamp, delay: ms }
  ...
  // Ref to store current pulse timeout to prevent race conditions
  ...
  const { postMessage } = useVscode(async (message: any) => {
  ...
  const bufferSocket = getWebSocket();
  ...
  const getOpacityForLayer = (layerName: string) => {
  ...
  const spawnParticles = useCallback((position: THREE.Vector3) => {
  ...
  const triggerEntanglement = useCallback((nodeId: string, axis: string) => {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      (wireMat as THREE.ShaderMaterial).uniforms.uTime.value = elapsed;
      (wireMat as THREE.ShaderMaterial).uniforms.uSpoonPct.value = Math.max(0, Math.min(1, spoonPct));
      (wireMat as THREE.ShaderMaterial).uniforms.uBreathPhase.value = breathIntensity;
    ...
    return () => {
      ...
      if (haloMeshRef.current) {
        ...
        (haloMeshRef.current.material as THREE.Material).dispose();
      }
      haloGeo.dispose();
      haloMat.dispose();
      ghostMat.dispose();
  ...
  return (
    <div className={`p31-container ${stressClass}`}>
      ...
      <div style={{ opacity: telOpacity, transition: 'opacity 0.8s ease-in-out' }}>
        ...
        <button
          style={{ position: 'absolute', top: 130, left: 10, padding: '5px', background: '#4ecdc4', color: '#000', cursor: 'pointer', border: 'none', borderRadius: '3px', fontSize: '10px', zIndex: 100 }}
