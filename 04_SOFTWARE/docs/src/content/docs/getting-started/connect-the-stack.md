...
| Observatory / dome lattice | [p31ca.org/observatory.html](https://p31ca.org/observatory.html) → [dome (Astro)](https://p31ca.org/dome) | Geodesic dome viz: search, axis/state filters, node detail (`src/pages/dome.astro`; legacy `/dome.html` redirects to `/dome`) |
