...
describe("Ledger Worker Protocol", () => {
  it("should initialize via persistence store", async () => {
    const { useLedgerStore } = await import("../stores/ledgerStore");

    // Trigger initialize — uses persistence store, not Worker
    try {
      await useLedgerStore.getState().initialize();
    } catch {
      // Expected — PGLite not available in test env
    }

    // Verify store attempted initialization (isLoading resets after try/catch)
    const state = useLedgerStore.getState();
    expect(state.isLoading).toBe(false);
  });
});
