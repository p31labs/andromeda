/**
 * SpIn Mesh Demo — Node B (Bob)
 *
 * Bob owns a Raspberry Pi 400 and wants Zelda (Minish Cap).
 */
...
const USER_ID = CONFIG.USER_ID_B;
...
async function main() {
  const db = new PGLite('spin-bob.db');
  await db.connect();
  await db.exec(`
  ...
  const rpiDoc = {
    ...
    id: RESOURCE_ID_RPI,
    type: "SpInResource",
    physical_format: "SingleBoardComputer",
    title: "Raspberry Pi 400",
    neuro_metadata: { monotropic_potential: 0.8, executive_function_requirement: "low", multiplayer_anxiety_index: 0.0, stimulation_profile: ["retro-computing", "keyboard-input"] },
  ...
  await db.prepare('INSERT INTO resources VALUES ($1, $2) ON CONFLICT(id) DO UPDATE SET doc = $2')
    .bind(RESOURCE_ID_RPI, JSON.stringify(rpiDoc)).run();
  ...
  `).bind(USER_ID, RESOURCE_ID_RPI, RESOURCE_ID_ZELDA).run();

  console.log('🔹 Bob: posted intent (Raspberry Pi 400 → Zelda)');
  ...
  console.log('🔹 Bob: cycle locked!', cycle.cycle_id);

   // Handover via Logistics DO
   console.log('🔹 Bob: starting handover with Logistics DO...');
   const handoverId = cycle.cycle_id;
   const expectedResourceIds = [RESOURCE_ID_RPI, RESOURCE_ID_ZELDA]; // What Bob posted
   ...
   console.log('🔹 Bob: submitting ephemeral public key to Logistics DO');
   ...
   console.log('🔹 Bob: submitted public key');

   // 3. Poll for ready (remote key available)
   console.log('🔹 Bob: waiting for remote public key...');
     ...
     try {
       ...
       if (!ready) {
         ...
         if (readyAttempts % 5 === 0) {
           console.log(`🔹 Bob: still waiting for ready... (${readyAttempts}/${MAX_READY_ATTEMPTS})`);
         }
       }
     } catch (error) {
       console.error('🔹 Bob: error in ready polling:', error.message);
       readyAttempts++;
   ...
   console.log('🔹 Bob: remote public key received');
   console.log(`🔹 Bob: handover details - cycle: ${ready.cycleId}, resources: ${ready.resourceIds.join(', ')}`);
   ...
   console.log('🔹 Bob: shared secret derived (size:', sharedSecretRaw.byteLength, 'bytes)');

   // 5. Complete handover
   console.log('🔹 Bob: completing handover...');
   ...
   if (compData.ok && compData.completed === 2) { // Expecting 2 participants in 2-party cycle
     console.log('🔹 Bob: handover complete — L.O.V.E. minted');
   } else if (compData.ok) {
     console.log(`🔹 Bob: handover step complete (${compData.completed} participants done)`);
   } else {
     console.error('🔹 Bob: handover failed', compData);
   }
