#!/usr/bin/env node
/**
 * Generates all *-about.html files in public/ from the registry below.
...
// ─── App registry ──────────────────────────────────────────────────────────
// Each entry: { id, title, tagline, icon, accent, status, statusLabel,
//               appUrl, tech[], features[], howTo[], techNotes, related[] }
const registry = [
  {
  ...
  },
  {
    ...
    related: ['ede', 'spaceship-earth', 'guardian']
  },
  {
  ...
  },
  {
    ...
    tech: ['Three.js r183', 'WebGL Bloom', 'ES Modules', 'Panel Inspector'],
    features: [
  ...
  // ── Research ────────────────────────────────────────────────────────────
  {
    ...
    icon: '🌀', accent: '#4db8a8', status: 'research', statusLabel: 'RESEARCH',
    appUrl: 'quantum-core.html',
    tech: ['FIPS-203 ML-KEM-768', 'SIC-POVM Swarm', 'IBM Quantum', 'Post-Quantum Primitives'],
    features: [
      'ML-KEM-768 (FIPS-203) key encapsulation — post-quantum cryptography primitives',
      'SIC-POVM swarm management: 1-qubit POVM with 4 elements, IC-POVM for 7-node mesh',
      'IBM Quantum bridge: execute real circuits against IBM hardware via the cloud API',
      'Lattice-based signature scheme primitives aligned with NIST PQC standards',
      'abdicate.sh integration point: key destruction protocol for governance transitions'
    ],
    howTo: [
      'Open Quantum Core — the SIC-POVM viewer loads the 4-element measurement set',
      'Switch to IBM Quantum tab to run a circuit — requires an IBM Quantum API key',
      'Inspect the ML-KEM-768 primitives in the Crypto tab — generate a test keypair'
    ],
    techNotes: 'IBM Quantum API calls are proxied through a Cloudflare Worker to avoid CORS. ML-KEM-768 is implemented in TypeScript following the FIPS-203 reference specification.',
