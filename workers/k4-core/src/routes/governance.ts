...
export class GovernanceEngineDO {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === '/health' && request.method === 'GET') return json({ ok: true, service: 'GovernanceEngineDO', ts: Date.now() });
  ...
  async createProposal(request: Request): Promise<Response> {
    ...
    if (!title || !description || !param || !target || !creatorDid) return err('Missing fields: title, description, param, target, creatorDid');
  ...
  async getProposal(id: string): Promise<Response> {
    ...
    if (!raw) return err('Proposal not found', 404);
  ...
  async deleteProposal(id: string, _request: Request): Promise<Response> {
    ...
    if (!raw) return err('Proposal not found', 404);
    const proposal: any = JSON.parse(raw as string);
    if (proposal.status !== PROPOSAL_STATUS.ACTIVE) return err('Only active proposals can be deleted');
    proposal.status = PROPOSAL_STATUS.REJECTED;
    proposal.executedAt = Date.now();
    await this.ctx.storage.put(KV.proposalDetail(id), JSON.stringify(proposal));
    return json({ ok: true, status: PROPOSAL_STATUS.REJECTED });
  }
  async castVote(proposalId: string, request: Request): Promise<Response> {
    const body = await request.json().catch(() => ({})) as Record<string, unknown>;
    const { voterDid, vote, stakeAmount } = body;
    if (!voterDid || !vote) return err('Missing voterDid or vote');
    if (!Object.values(VOTE).includes(vote as VoteValue)) return err('Vote must be yes/no/abstain');
    const proposalRaw = await this.ctx.storage.get(KV.proposalDetail(proposalId));
    if (!proposalRaw) return err('Proposal not found', 404);
    const proposal: any = JSON.parse(proposalRaw as string);
    if (proposal.status !== PROPOSAL_STATUS.ACTIVE) return err('Proposal is not active');
    if (Date.now() > proposal.closesAt) return err('Voting period has ended');
    const voteKey = KV.voteKey(proposalId, String(voterDid));
    const existing = await this.ctx.storage.get(voteKey);
    if (existing) return err('Already voted on this proposal');
    let lockedStake = 0;
    ...
    return json({ ok: true, proposal, vote: voteRecord });
  }
  async executeProposal(id: string): Promise<Response> {
    const raw = await this.ctx.storage.get(KV.proposalDetail(id));
    if (!raw) return err('Proposal not found', 404);
    const proposal: any = JSON.parse(raw as string);
    if (proposal.status !== PROPOSAL_STATUS.ACTIVE) return err('Already resolved');
    const vetoRaw = await this.ctx.storage.get(KV.vetoKey(id));
    if (vetoRaw) {
      proposal.status = PROPOSAL_STATUS.VETOED;
      proposal.executedAt = Date.now();
      await this.ctx.storage.put(KV.proposalDetail(id), JSON.stringify(proposal));
      return json({ ok: true, status: PROPOSAL_STATUS.VETOED, proposal });
    }
    ...
    return json({ ok: true, status: proposal.status, proposal });
  }
  async vetoProposal(id: string, _request: Request): Promise<Response> {
    const raw = await this.ctx.storage.get(KV.proposalDetail(id));
    if (!raw) return err('Proposal not found', 404);
    const proposal: any = JSON.parse(raw as string);
    if (Date.now() > proposal.vetoDeadline) return err('Veto window has expired');
    await this.ctx.storage.put(KV.vetoKey(id), JSON.stringify({ vetoedAt: Date.now() }));
    return json({ ok: true, vetoed: true, proposalId: id });
  }
  async upsertDelegation(request: Request): Promise<Response> {
    const body = await request.json().catch(() => ({})) as Record<string, unknown>;
    const { delegatorDid, delegateDid, expiresAt } = body;
    if (!delegatorDid || !delegateDid) return err('Missing delegatorDid or delegateDid');
    ...
    return json({ ok: true, delegation: record });
  }
  async getDelegationRequest(request: Request): Promise<Response> {
    const url = new URL(request.url);
    const delegatorDid = url.pathname.split('/').pop();
    if (!delegatorDid) return err('Missing delegatorDid');
    const raw = await this.ctx.storage.get(`gov:delegation:${delegatorDid}`);
    if (!raw) return err('Delegation not found', 404);
    return json(JSON.parse(raw as string));
  }
...
const app = new Hono();
app.get('/health', (c) => c.json({ ok: true, service: 'governance-engine', version: '0.1.0', ts: Date.now(), pqc: { compliant: (c.env as any).PQC_COMPLIANT === 'true', algorithms: (c.env as any).PQC_ALGORITHMS, standard: (c.env as any).PQC_STANDARD } }));

app.route('/proposals', new Hono()
  .post('/', async (c) => {
    const stub = (c.env as any).GOVERNANCE_ENGINE_DO.get((c.env as any).GOVERNANCE_ENGINE_DO.idFromName('governance'));
  ...
  .patch('/:id/execute', async (c) => {
    const stub = (c.env as any).GOVERNANCE_ENGINE_DO.get((c.env as any).GOVERNANCE_ENGINE_DO.idFromName('governance'));
    return stub.fetch(c.req.raw);
  })
  .post('/:id/veto', async (c) => {
    const stub = (c.env as any).GOVERNANCE_ENGINE_DO.get((c.env as any).GOVERNANCE_ENGINE_DO.idFromName('governance'));
    return stub.fetch(c.req.raw);
  })
);

app.route('/delegations', new Hono()
  .post('/', async (c) => {
    const stub = (c.env as any).GOVERNANCE_ENGINE_DO.get((c.env as any).GOVERNANCE_ENGINE_DO.idFromName('governance'));
    return stub.fetch(c.req.raw);
  })
