...
Copy these 5 files into `src/config/`:
...
| `bashium.ts` | Secret element config + all text content |
...
### 6. Bashium — Secret Element

This is the biggest integration. Steps:

**a) Add Bashium to elements config** (wherever H, O, C, etc. are defined):
```typescript
import { BASHIUM_ELEMENT } from '../config/bashium';

// In the elements array, at the end:
// Only include if genesis quest is complete
const elements = [
  ...baseElements,
  ...(genesisComplete ? [BASHIUM_ELEMENT] : []),
];
```

**b) Gate palette visibility:**
The palette should check `questProgress.genesis.chainComplete` before showing Ba.

**c) Wire unlock toast:**
When `chainComplete` flips to true for genesis, show the Bashium unlock toast:
```typescript
...
**d) Wire completion message:**
When any molecule containing Ba is completed:
```typescript
import { BASHIUM } from '../config/bashium';

// Check if completed molecule contains Bashium:
if (atoms.some(a => a.symbol === 'Ba')) {
  // Show special Bashium completion instead of normal:
  // BASHIUM.completionMessage.line1
  // BASHIUM.completionMessage.line2
  // BASHIUM.completionMessage.line3
  // + DOUBLE confetti
}
```
